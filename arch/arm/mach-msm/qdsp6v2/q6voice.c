...
static int voice_send_cvs_cal_to_modem(struct voice_data *v)
{
	...
	/*	
		//for a mediaserver crash
		ret = copy_from_user(cmd_buf + (APR_HDR_SIZE / sizeof(uint32_t)),
				            cal_data_per_network, cal_size_per_network);
		if(!ret){
			v->cvs_state = CMD_STATUS_FAIL;
			kfree(cmd_buf);
			pr_err("%s copy_from_user fault",__func__);
			return -EFAULT;
		}
	*/	
...
static int voice_send_cvp_cal_to_modem(struct voice_data *v)
{
	...
	for (; index < cal_data.num_cal_blocks; index++) {
	   ...
	   /*
		//for a mediaserver crash		
		ret = copy_from_user(cmd_buf + (APR_HDR_SIZE / sizeof(*cmd_buf)),
				     cal_data_per_network, cal_size_per_network);
		        if(!ret){
				   v->cvs_state = CMD_STATUS_FAIL;
				   kfree(cmd_buf);
