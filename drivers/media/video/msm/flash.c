		...
		unsigned char *txdata, int length)
{
	...
	if (i2c_transfer(aat1272_client->adapter, msg, 1) < 0) {
		printk("aat1272_i2c_txdata faild 0x%x\n", saddr);
		return -EIO;
	}
...
static int32_t aat1272_init_i2c(void)
{
	...
	printk("%s aat1272_i2c_driver rc = %d\n",__func__, rc);
	if (IS_ERR_VALUE(rc))
		goto init_i2c_fail;
...
err1:
	...
	case MSM_CAMERA_LED_INIT:
		printk("%s MSM_CAMERA_LED_INIT E\n",__func__);
		aat1272_init_i2c();
		rc = gpio_request(4, "FLASH_CNTL_EN");
		if (!rc) {
		    printk("%s:%d FLASH_CNTL_EN INIT_MODE \n", __func__, __LINE__);
		...
		if (!rc) {
		    printk("%s:%d FLASH_CNTL_FLEN INIT_MODE \n", __func__, __LINE__);
		    gpio_direction_output(3, 0);
		} else {
			printk("%s: gpio FLASH_CNTL_FLEN request fail", __func__);
			goto err1;
		}
		printk("%s MSM_CAMERA_LED_INIT X\n",__func__);
		return rc;
err1:
	...
	case MSM_CAMERA_LED_RELEASE:
		printk("%s MSM_CAMERA_LED_RELEASE E\n",__func__);
		...
		i2c_del_driver(&aat1272_i2c_driver);
		aat1272_client = NULL;
		printk("%s MSM_CAMERA_LED_RELEASE X\n",__func__);
		break;

	case MSM_CAMERA_LED_OFF:
		printk("%s MSM_CAMERA_LED_OFF E\n",__func__);
		gpio_direction_output(3, 0);
		aat1272_i2c_write_b_flash(0x01, 0x03);
		printk("%s MSM_CAMERA_LED_OFF X\n",__func__);
		break;

	case MSM_CAMERA_LED_LOW:
		printk("%s MSM_CAMERA_LED_LOW E\n",__func__);
		gpio_direction_output(3, 1);
		aat1272_i2c_write_b_flash(0x00, 0x20);
		aat1272_i2c_write_b_flash(0x01, 0x31);
		printk("%s MSM_CAMERA_LED_LOW X\n",__func__);
		break;

	case MSM_CAMERA_LED_HIGH:
		printk("%s MSM_CAMERA_LED_HIGH E\n",__func__);
		gpio_direction_output(3, 1);
		printk("%s MSM_CAMERA_LED_HIGH X\n",__func__);
		break;
...
err1:
	...
	case MSM_CAMERA_LED_HIGH:
		...
		tps61050_i2c_write_b_flash(0x01, 0x86);
		gpio_direction_output(3, 1);
#if 0 //Timing issue		
		mdelay(200);
#endif
