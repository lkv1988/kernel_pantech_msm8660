					...
					enum led_brightness value)
{
	...
	int bl_lvl;
	printk("msm_fb_set_bl_brightness:%03d\n", value);
...
int msm_reset_get_bl(void)
{
	...
	restart_bl = ioremap_nocache(SYS_RESTART_REASON_ADDR, 0x1000); //pz1714 20120313 chg
...
int msm_reset_reason_read_only(void)
{
	...
	restart_reason = ioremap_nocache(SYS_RESTART_REASON_ADDR, 0x1000); //pz1714 20120313 chg
	...
	MSM_FB_ERR("[LIVED] msm_reset_reason_read_only:reason[%x],result[%x]\n",reason,result);
	return result;
}
...
#if defined(CONFIG_EF33_BOARD) || defined(CONFIG_EF34_BOARD) || defined(CONFIG_EF35_BOARD) || defined(CONFIG_VEGACU_BOARD)
			...
			unsigned long arg)
{
	...
	case MSMFB_SKY_LCD_FORCE_ONOFF:
		...
		bl_updated = 1; //GCAN + In ICS, insert for Offline charging mode backlight control 
		up(&msm_fb_ioctl_ppp_sem);
		break;
#endif
#ifdef F_SKYDISP_LCD_GAMMA_TEST
	case MSMFB_SKY_LCD_GAMMA_SET:
		...
		MSM_FB_ERR("[LIVED] LCD Gamma Set gamma=%d\n", gamma);
		down(&msm_fb_ioctl_ppp_sem);
