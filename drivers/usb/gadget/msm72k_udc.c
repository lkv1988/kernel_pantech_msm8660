...
#ifdef CONFIG_SKY_CHARGING
	factory_check = pm8058_is_factory_cable();
	dev_info(&ui->pdev->dev, "[SKY CHG]jcpark 1 suspended = %d, temp %d, factory_check = %d\n", suspended, temp, factory_check);
	if (factory_check){
		return 1500;//USB_WALLCHARGER_CHG_CURRENT;
	}

	dev_info(&ui->pdev->dev, "[SKY CHG]jcpark 2 suspended = %d, temp %d ui->usb_state = %d\n", suspended, temp, ui->usb_state);
	if(suspended && (temp == USB_CHG_TYPE__SDP)){
		return 600;
	}
