...
static wait_queue_head_t icp_hd_wait_queue;


static DECLARE_WAIT_QUEUE_HEAD(icp_hd_wait_queue);
DEFINE_SEMAPHORE(icp_hd_sem);
...
static int icp_hd_vreg_init(void)
{
	...
	if (IS_ERR(s2b_1p2v)) {
		...
		CAM_ERR("%s: regulator_get s2b_1p2v fail \n", __func__);
		return -ENODEV;
	}
	...
	if (IS_ERR(mvs0b_1p8v)) {
		...
		CAM_ERR("%s: regulator_get mvs0b_1p8v ail\n", __func__);
		return -ENODEV;
	}
...
//	int32_t i;
	...
	if (rc < 0)
	{
		CAM_ERR("%s ERR: FAIL!!!rc=%ld return~~\n", __func__, rc);
		return rc;
	}
...
//	uint16_t reg_val;
	...
	if (rc < 0)
	{
		CAM_ERR("ERR:%s FAIL!!!rc=%ld return~~\n", __func__, rc);
		return rc;
	}		
...
#ifdef F_PANTECH_CAMERA_FIX_CFG_ISO
static int32_t icp_hd_set_iso(int32_t iso)
{
	/* auto, N/A, ISO100, ISO200, ISO400, ISO800, ISO1600, ISO3200 */
	//uint8_t data[] = {0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06};//pangya compile error tmp
	int32_t rc = 0;
...
//	uint16_t clock;
	...
	case SENSOR_PREVIEW_MODE:
		...
		if(config_csi == 0)
		{
...
#if defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_NODATA) || defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_2)	|| defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_3) || defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_4) || defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_5) || defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_6) || defined(BURST_MODE_8192_BYTE_COMPRESS_INIT_OTP_TUNE_UPDATE_7)
            ...
            for (i = 0; i < ICP_HD_OTP_UPLOADING_POLLING_RETRY; i++) 
            {
                ...
                if (rc < 0)
                {
                    CAM_INFO("OTP UPLOADING POLLING READ I2C FAIL!!! return~~ rc=%ld\n", rc);
                    return rc;
                }
		...
		else
		{
			...
			CAM_INFO("icp_hd_video_config, rc = %ld \n", rc);
			if (rc < 0)
	              {
				CAM_ERR("ERR:SENSOR_PREVIEW_MODE FAIL!!!rc=%ld return~~\n", rc);
				return rc;
			}
		}
		break;
	...
	case SENSOR_SNAPSHOT_MODE:
		if(!b_snapshot_flag) {
			...
			{	
				...
				icp_hd_set_led_mode(ICP_HD_CFG_LED_MODE_SNAP);
					...
					for (i = 0; i < ICP_HD_LED_HALF_SHUTTER_POLLING_RETRY; i++) 
					{
						...
						if (rc < 0)
						{
							CAM_INFO("LED HALF SHUTTER POLLING READ I2C FAIL!!! return~~ rc=%ld\n", rc);
							return rc;
						}
				...
				for (i = 0; i < ICP_HD_ANTISHAKE_HALF_SHUTTER_POLLING_RETRY; i++) 
					{
						...
						if (rc < 0)
						{
							CAM_INFO("ANTISHAKE HALF SHUTTER POLLING READ I2C FAIL!!! return~~ rc=%ld\n", rc);
							return rc;
						}
			...
			CAM_INFO("icp_hd_snapshot_config, rc = %ld \n", rc);
			if (rc < 0)
	              {
				CAM_ERR("ERR:SENSOR_SNAPSHOT_MODE FAIL!!! rc=%ld return~~\n", rc);
				return rc;
			}			
		}
		break;
...
//	uint16_t model_id = 0;
	...
	rc = icp_hd_reg_init();
	CAM_INFO("icp_hd_reg_init X - rc : %d \n", rc);
	if (rc < 0)
		goto init_probe_fail;
