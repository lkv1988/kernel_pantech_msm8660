...
void init_pantech_debug_option(void)
{
	...
	debug_option_type	test_flag;;

	printk(KERN_ERR "%s is called\n", __func__);

	debug_flag.bytes = 0x07;
	
	// pz1714 modem rpc test
	r = msm_nv_read(NV_SPARE_1_I, (nv_cmd_item_type *)&test_flag.bytes);
	if (r != NV_DONE_S) { 
		printk(KERN_ERR "%s.%d: msm_nv_read error(%d)\n", __func__, __LINE__, r);
	}
	else
	{
		printk(KERN_ERR "%s.%d: is called debug_flag.bytes:%x\n", __func__,__LINE__, test_flag.bytes);
	}
#else
...
static void set_pantech_debug_option(struct pansw_classdev *sw_dev, int value)
{
	...
	msm_nv_r = msm_nv_write(NV_SPARE_1_I, (nv_cmd_item_type *)&debug_flag.bytes);
	printk(KERN_ERR "%s is called debug_flag.bytes:%x\n", __func__,debug_flag.bytes);

	if (msm_nv_r != NV_DONE_S) { 
		// minus : RPC error, plus : NV error
