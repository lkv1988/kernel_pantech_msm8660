...
// jjhun_20120307_sound_kernel_merge  >>>>
#define PLAYBACK 0x1
					...
					int rate, int mode)
{
	...
	if (set) {
#ifdef CONFIG_RECINCALL
		rc = adm_open(copp_id, ADM_PATH_PLAYBACK, rate, mode, DEFAULT_COPP_TOPOLOGY); // jjhun GB PLAYBACK
// jjhun_20120307_sound_kernel_merge  >>>>
#else
...
#ifdef CONFIG_RECINCALL
		rc = adm_open(copp_id, ADM_PATH_LIVE_REC, rate, mode, topology); // jjhun GB : LIVE_RECORDING
// jjhun_20120307_sound_kernel_merge  >>>>
#else
		rc = adm_open(copp_id, popp_id, LIVE_RECORDING, rate, mode, topology);
// jjhun_20120307_sound_kernel_merge  <<<<		
#endif
