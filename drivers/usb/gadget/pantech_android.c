...
static ssize_t usb_serial_write_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
 ...
 // memcpy(pantech_serial_number + strlen(SYS_MODEL_NAME), buf, 14);
  memcpy(pantech_serial_number, "EF33S", strlen("EF33S"));
  memcpy(pantech_serial_number + strlen("EF33S"), buf, 14);
...
static ssize_t usb_charging_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	b_pantech_usb_module = true; //pdj666_test
	printk(KERN_ERR "###PDJ666[%s]name[%s], b_pantech_usb_module:%d\n", __func__, buf,b_pantech_usb_module);
