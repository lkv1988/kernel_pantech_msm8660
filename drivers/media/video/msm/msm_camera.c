...
static int msm_open_control(struct inode *inode, struct file *filep)
{
	...
	pr_err("[pangya] %s : g_v4l2_opencnt = %d\n", __func__,g_v4l2_opencnt);

//pangya _120223 Start [[ failed control release. 
#if 0
	if (g_v4l2_opencnt){		
		struct msm_cam_device *pmsm_control = g_v4l2_control_device ->pmsm;
		
		g_v4l2_opencnt--;		
		pr_info("[pangya]%s, g_v4l2_opencnt = %d: msm_release_control called to force\n", __func__,g_v4l2_opencnt);
		
		mutex_lock(&pmsm_control->sync->lock);
		if (pmsm_control->sync->core_powered_on && pmsm_control->sync->vfefn.vfe_stop) {
			pr_info("%s, stop vfe if active\n", __func__);
			pmsm_control->sync->vfefn.vfe_stop();
		}
		mutex_unlock(&pmsm_control->sync->lock);
		rc = __msm_release(pmsm_control->sync);
		if (!rc) {
			msm_queue_drain(&g_v4l2_control_device->ctrl_q, list_control);
			kfree(g_v4l2_control_device);
		}
#ifdef F_SKYCAM_FIX_MSM_OPEN_FAIL
		if (!rc) {
			msm_queue_drain(&pmsm_control->sync->event_q, list_config);
			atomic_set(&pmsm_control->opened, 0);
		}
#endif    
	}
#endif	
