...
//--> DS1_DS_WIFI_IWLAN_PORTING
	...
	return  addr_match(&fl4->daddr, &sel->daddr, sel->prefixlen_d) &&
		...
		(sel->dport_end ? ((xfrm_flowi_dport(fl, &fl4->uli) >= (sel->dport & sel->dport_mask)) &&
		(xfrm_flowi_dport(fl, &fl4->uli) <= (sel->dport_end & sel->dport_mask))) :
		!((xfrm_flowi_dport(fl, &fl4->uli) ^ sel->dport) & sel->dport_mask)) &&
		(sel->sport_end ? ((xfrm_flowi_sport(fl, &fl4->uli) >= (sel->sport & sel->sport_mask)) &&
		(xfrm_flowi_sport(fl, &fl4->uli) <= (sel->sport_end & sel->sport_mask))) :
